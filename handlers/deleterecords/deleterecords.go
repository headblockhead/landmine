...
func (h Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	req, validationFailures := models.NewDeleteRecordsRequest(r)
	...
	resp, err := h.backend.DeleteMultiple(r.Context(), req)
	if err != nil {
		h.log.Error("failed to delete records", slog.Any("error", err))
		respond.WithJSON(w, models.NewError("failed to delete records", http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
